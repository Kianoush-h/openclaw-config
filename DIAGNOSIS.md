...
- **Fix applied:**
  - `openclaw cron edit <roaster> --no-deliver` and `… <competitor> --no-deliver` → `delivery.mode: none`. Competitor is silent on `#top5engineering` (verified). **⚠️ Superseded for the roaster by #13:** `--no-deliver` assumed the agent always tool-posts, but it doesn't reliably — this caused a silent no-post on 07-02. The roaster was switched to **announce-only + output-as-reply** (see #13). Competitor keeps `--no-deliver` because it posts to a *different* channel via the tool.
...
- **Fix applied (per user: data-driven + holiday skip), in two passes:**
  - Deleted the bad 07-01 post; rewrote the roaster prompt with a **date/holiday gate** (weekend + listed 2026 Ontario statutory holidays → `NO_REPLY`), **strict same-day matching**, and an **empty-day gate** (zero posts → `NO_REPLY`).
  - **Delivery correction (critical):** the first pass left delivery `mode: none` (from #12) assuming the agent posts via the message tool. On the 2026-07-02 09:35 run the agent instead produced the roaster as its **final reply** and did **not** call message-send — so with announce off, **nothing posted** (`deliveryStatus: not-requested`). Corrected by making delivery **announce-only**: `openclaw cron edit … --announce --channel slack --to C02DCBU9T7U`, and changing the prompt to **output the summary as its reply (never tool-post)**, `NO_REPLY` to skip. Now exactly one post per run, deterministically.
  - **Accuracy correction:** the default `gemini-3.1-flash-lite` excluded posters who **typo'd the date** in their text ("2026-06-02" for 07-02), wrongly flagging them "No post yet". Fixed by (a) instructing the model to match "today" by the Slack **`ts`** timestamp, not the typed date, with a concrete example, and (b) switching the job to **`openrouter/x-ai/grok-4.3`** (follows the instruction reliably).
- **Verified:** manual run on 2026-07-02 posted a complete `[2026-07-02]` roaster including **all 5** members (typo'd dates correctly ignored via `ts`), "No post yet: None", delivered via announce. Kept as today's roaster.
- **Key learning:** (1) a scheduled "report from chat activity" agent must gate on *real, same-day* data (weekend/holiday/empty gates) or it fabricates/echoes stale data on off days. (2) For a same-channel job, **announce-only + output-as-reply** is far more reliable than relying on the agent to call message-send (`--no-deliver` can silently drop the post — see #12 correction). (3) Cheap models mishandle conflicting cues (Slack `ts` vs typo'd text date); use a stronger model + an explicit example for accuracy-critical jobs. See `docs/06`.
