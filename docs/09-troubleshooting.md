...
| `TaskFlow recovery … blocked TaskFlow points at missing task` | Orphaned TaskFlows reference deleted tasks | Best: `openclaw tasks audit` to inspect, then `openclaw tasks maintenance` (preview) and `--apply` to reconcile+prune all terminal flow records. Surgical alt: `openclaw tasks flow cancel <flow-id>`. Flow records are bookkeeping — pruning doesn't undo real outputs |
